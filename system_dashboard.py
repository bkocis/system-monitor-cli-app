...
class SystemDashboard:
    def __init__(self):
        ...
        self.max_history_points = 100  # Keep last 60 readings (~1 minute if updating every second)
            ...
            if result.returncode == 0:
                ...
                if len(data) >= 6:
                    return {
                        ...
                        'power_draw': float(data[5]) if data[5].replace('.', '').isdigit() else None
                    }
        except:
            pass
        return {}
    ...
    def update_temperature_history(self, gpu_temp, cpu_temp):
        ...
        if gpu_temp is not None:
            ...
            if len(self.temp_history['gpu']) > self.max_history_points:
                self.temp_history['gpu'] = self.temp_history['gpu'][-self.max_history_points:]
        ...
        if cpu_temp is not None:
            ...
            if len(self.temp_history['cpu']) > self.max_history_points:
                self.temp_history['cpu'] = self.temp_history['cpu'][-self.max_history_points:]
    ...
    def create_temperature_graph(self, temp_type, height=8, length=100):
        ...
        temp_color = "green" if current_temp < 70 else "yellow" if current_temp < 80 else "red"
    ...
    def create_dashboard(self):
        ...
        if cpu_temp is not None:
            cpu_color = self.get_color_for_percentage(cpu_temp * 100 / 90)  # assuming 90°C as max
            cpu_current = f"[{cpu_color}]{cpu_temp}°C[/{cpu_color}]"
        else:
            cpu_current = "[red]N/A[/red]"
        
        cpu_temp_graph = self.create_temperature_graph('cpu', height=6, length=available_graph_width)
        temp_table.add_row("🏭 CPU:", cpu_current, cpu_temp_graph)
        
        # GPU row with current temp and history  
        if gpu_temp is not None:
            gpu_color = self.get_color_for_percentage(gpu_temp * 100 / 90)  # assuming 90°C as max
            gpu_current = f"[{gpu_color}]{gpu_temp}°C[/{gpu_color}]"
        else:
            gpu_current = "[red]N/A[/red]"
            
        gpu_temp_graph = self.create_temperature_graph('gpu', height=6, length=available_graph_width)
        ...
        sys_table.add_row("💻 CPU Usage:", f"[{cpu_color}]{cpu_info['percent']:.1f}%[/{cpu_color}]")
        sys_table.add_row("🔢 CPU Cores:", f"{cpu_info['count']}")
        
        mem_color = self.get_color_for_percentage(memory_info['percent'])
        sys_table.add_row("🧠 Memory:", f"[{mem_color}]{memory_info['percent']:.1f}%[/{mem_color}]")
        sys_table.add_row("📦 Used/Total:", f"{self.format_bytes(memory_info['used'])} / {self.format_bytes(memory_info['total'])}")
        ...
        if gpu_info:
            ...
            if gpu_info.get('utilization') is not None:
                ...
                gpu_table.add_row("⚡ Usage:", f"[{util_color}]{gpu_info['utilization']}%[/{util_color}]")
            if gpu_info.get('memory_used') is not None and gpu_info.get('memory_total') is not None:
                mem_percent = (gpu_info['memory_used'] / gpu_info['memory_total']) * 100
                mem_color = self.get_color_for_percentage(mem_percent)
                gpu_table.add_row("🧠 VRAM:", f"[{mem_color}]{gpu_info['memory_used']}MB / {gpu_info['memory_total']}MB[/{mem_color}]")
        ...
        for disk in disk_info:  # Show real physical drives only
            ...
            if (device_name.startswith('/dev/loop') or 
                device_name.startswith('/dev/snap') or
                disk['fstype'] in ['tmpfs', 'devtmpfs', 'sysfs', 'proc', 'cgroup', 'devpts', 'securityfs', 'pstore', 'efivarfs', 'bpf', 'cgroup2', 'configfs', 'debugfs', 'tracefs', 'fusectl', 'mqueue', 'hugetlbfs'] or
                mountpoint.startswith('/sys') or
                mountpoint.startswith('/proc') or
                mountpoint.startswith('/dev') or
                mountpoint.startswith('/run') or
                mountpoint.startswith('/snap') or
                mountpoint.startswith('/var/snap/') or
                mountpoint == '/boot/efi' or
        ...
        dashboard = Group(header_panel, temp_panel, sys_panel, gpu_panel, disk_panel, footer_panel)
